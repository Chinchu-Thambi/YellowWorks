...
const ConfirmV2 = ({ subscriptionId, location }) => {
  // load auth utilities
  const {
  ...
  const subscriptions = businessStore.getPath(['subscriptions', 'edges']) || [];
  const subscription = R.find(R.propEq('id', subscriptionId))(subscriptions);

  // Needs SKU not provider
  const steps = getSteps({
    skuList: [{ value: R.toUpper(subscription?.provider || 'WEBSITE') }],
  });
  ...
  switch (coreProductSku) {
    case 'YPBASIC':
      ConfirmHandler = YolConfirm;
      break;
    case 'YPPREMIUM':
      ConfirmHandler = YolConfirm;
      break;
    case 'GMB':
      ConfirmHandler = GmbConfirm;
      break;
    case 'WEBSITE':
      ConfirmHandler = WebsiteConfirm;
      break;
    default:
