...
describe('FullImageLeftTemplate', () => {
  it('renders from props', () => {
    ...
    const { container } = render(<FullImageLeftTemplate {...props} />);
  ...
  it('renders case study', () => {
    ...
    // eslint-disable-next-line react/jsx-props-no-spreading
    render(<FullImageLeftTemplate {...props} />);

    expect(screen.getByText(caseStudy.text).closest(`[href="${caseStudy.link}"]`))
      .toBeInTheDocument();
  });
});
